...
class tb_scoreboard extends uvm_scoreboard;
    ...
    virtual function write(seq_item item);
        ...
        model.get_output(model_output.vector, model_output.result, model_output.row_index, model_output.found);

        if (!item.compare(model_output)) begin
            error_flags = '{
                item.result != model_output.result  ,
                item.row_index != model_output.row_index  
            };

            if (error_flags[0]) begin
                `uvm_error(get_name(), $sformatf("\n[RESULT ERROR] vector = %012b: DUT = %012b, model = %012b"  ,
                                                    item.vector, item.result, model_output.result               ))
            end
            if (error_flags[1]) begin
                `uvm_error(get_name(), $sformatf("\n[INDEX ERROR] vector = %012b: DUT = %0d, model = %0d"   ,
                                                    item.vector, item.row_index, model_output.row_index     ))
            end
            item.print();
            model_output.print();
        end
